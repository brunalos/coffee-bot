...
async def select_drink_type(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    logger.info("User %s chose drink type: %s",
                update.message.from_user.id, user_input)

    if user_input.startswith('/cancel'):
        return await cancel(update, context)

    coffee_types = ['coffee', 'espresso',
    ...
    if user_input in coffee_types:
        ...
        logger.warning("User %s provided an invalid drink type: %s",
                       update.message.from_user.id, user_input)
        await update.message.reply_text(
...
async def intensity(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    user_input = update.message.text.lower()
    logger.info("User %s chose intensity: %s",
                update.message.from_user.id, user_input)
    ...
    if user_input in intensity_types:
        ...
        logger.warning("User %s provided an invalid intensity: %s",
                       update.message.from_user.id, user_input)
        await update.message.reply_text(
...
async def size_selection(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    reply_size = ['one', 'two']
    user_input = update.message.text.lower()
    logger.info("User %s chose coffee size: %s",
                update.message.from_user.id, user_input)
    ...
    if user_input in reply_size:
        ...
        logger.warning("User %s provided an invalid coffee size: %s",
                       update.message.from_user.id, user_input)
        await update.message.reply_text(
...
async def exit_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if user_input.startswith('/drink'):
        return await drink_type_command(update, context)
    else:
        await update.message.reply_text(
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    user = update.message.from_user
    await update.message.reply_text(
        "Goodbye! Have a nice coffeeless day!\n"
        "If you change your mind, feel free to start a new conversation by typing /start.", reply_markup=ReplyKeyboardRemove()
    )
    return ConversationHandler.END
...
def main() -> None:
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            select_drink_type: [MessageHandler(filters.TEXT, select_drink_type), CommandHandler('cancel', cancel)],
            intensity: [MessageHandler(filters.TEXT, intensity), CommandHandler('cancel', cancel)],
            size_selection: [MessageHandler(filters.TEXT, size_selection), CommandHandler('cancel', cancel)],
